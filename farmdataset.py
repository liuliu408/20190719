...
class FarmDataset(Dataset):
    def __init__(self, train_path, label_path, img_format="png", istrain=True, isaug=True):
        self.istrain = istrain
        self.trainxformat = os.path.join(train_path, "*.{}".format(img_format))
        self.trainyformat = os.path.join(label_path, "*.{}".format(img_format))
    ...
    def __getitem__(self, idx):
        if self.istrain:
            ...
            data_base = self.trainxformat.split('/')[-2]
            label_base = self.trainyformat.split('/')[-2]
            # print("\n data and label base", data_base, label_base)
            imgyname = imgxname.replace(data_base, label_base)
